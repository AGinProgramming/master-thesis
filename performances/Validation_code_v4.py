...
sel_method = 'rfe'
with open(r'Z:\GENIM\Hesheng Gong\best_models_'+ sel_method + '.pickle', 'rb') as f:
    best_models = pickle.load(f)
with open(r'Z:\GENIM\Hesheng Gong\feat_lists_'+ sel_method + '.pickle', 'rb') as f:
    feat_list = pickle.load(f)

# Load dMRI dataset (train) - WE WILL NOT USE THIS DATASET
...
# Load NKI dataset (test) - WILL BE USED FOR TEST AND TRAIN IN CROSS VALIDATION
...
f = open(r"Z:\GENIM\Hesheng Gong\features_HC_PCA_MRMR.txt", "r")
f_content = f.read()
...
for mtr in metrics:
    ...
    for mdl in best_models[mtr].keys():
        ...
        # Train model on dMRI dataset - TO BE CHANGED - we need to add a cross validation loop to train and test on the NKI dataset        
